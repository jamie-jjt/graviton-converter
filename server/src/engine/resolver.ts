...
export class AutoResolver {
  ...
  resolve(issue: ConversionIssue): ConversionIssue {
    ...
    try {
      ...
      if (cleanFix.includes('sse2neon.h')) {
        this.ensureSse2neon(path.dirname(filePath));
      }
   ...
   */
  private ensureSse2neon(targetDir: string): void {
    const sse2neonPath = path.join(this.basePath, 'sse2neon.h');
    if (fs.existsSync(sse2neonPath)) return;

    const localPath = path.join(targetDir, 'sse2neon.h');
...
#endif /* SSE2NEON_H */
`;
    fs.writeFileSync(sse2neonPath, header, 'utf-8');
  }
